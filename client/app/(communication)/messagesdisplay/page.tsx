...
export default function Page() {
      ...
      if (bearerToken) {
        ...
        try {
          const { data } = await axios.get(
            `http://localhost:8004/searchuser?search=${search}`,
            {
              headers: {
  ...
  return (
    ...
    <div className="flex flex-row   mb-3 w-full justify-center mt-3">
      ...
      />
      <button
        className="bg-green-600 p-1 rounded-e-md shadow-md border h-10"
    ...
    <div className="flex flex-col h-fit  w-full items-center">
      ...
      {chats ? (
        chats.map((x: any) => (
            <Link key={x._id} href={`/messages`}
            className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2"
            >
          <button
            onClick={() => dispatch(setSelectedChat(x))}
          >
            <img 
              src={getSenderImg(loggedUser, x.users)}
