import React from "react";

export default function ChatAllUsers() {
  return (
    <div className=" w-1/4 h-screen">
      <div className="flex flex-row  mb-3 w-full justify-center mt-3">
        <input
          type="text"
          className="pl-1 p-2 shadow-md outline-none rounded-s-md border bg-bodywhite"
          placeholder="search"
        />
        <button className="bg-green-600 p-1 rounded-e-md shadow-md border">
          Search
        </button>
      </div>
      <div className="flex flex-col h-screen  w-full items-center  overflow-x-scroll">
        {/* users */}
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        <div className="bg-bodywhite  h-16 rounded-md shadow-md w-11/12 flex border  mb-2 items-center p-2">
          <div className="h-10 w-10 bg-black rounded-3xl"></div>
          <div className="ml-2">
            <div className="text-md font-bold">Seller name</div>
            <div className="text-sm font-semibold">How can i help you? </div>
          </div>
          <div>
          </div>
        </div>
        
       
        
      </div>
      <div>
      </div>
    </div>
  );
}
